...
void DWM1000_Tag::wiring() {
  ...
  pollTimer >> ([&](const TimerMsg& tm) { _pollTimerExpired = true; });
  logTimer >> ([&](const TimerMsg& tm) {
    INFO(
        "interr: %d TO:%d blink: %d poll: %d resp: %d final:%d anchors: %d "
        "delay:%d usec",
        _interrupts, _timeouts, _blinks, _polls, _resps, _finals,
        anchorsCount(), _interruptDelay);
    for (int i = 0; i < MAX_ANCHORS; i++) {
      if (anchors[i]._address != 0) {
        ...
        message =
            stringFormat("{\"x\":%d,\"y\":%d,\"distance\":%d}", anchors[i]._x,
  ...
  checkTimer >> ([&](const TimerMsg& tm) {
    ...
    if (oldInterrupts == _interrupts) {
      INFO(" missing interrupts , lastEvent : %d lastStatus : 0x%X", lastEvent,
           lastStatus);
  ...
  expireTimer >> ([&](const TimerMsg& tm) { expireAnchors(); });
}
...
void DWM1000_Tag::diag(const char* msg) {
  ...
  INFO(" %s SYS_MASK : %X SYS_STATUS : %X SYS_STATE: %X state : %s IRQ : %d",
       msg, sys_mask, sys_status, sys_state, stateString(), _irq.read());
}
...
void DWM1000_Tag::updateAnchors(BlinkMsg& blinkMsg) {
  ...
  if ((rap = anchorsFind(address)) == 0) {
    ...
    INFO_ISR(" +1 anchor : %d x:%d y:%d dist: %d", address, rap->_x, rap->_y,
             rap->_distance);
  } else {
    rap->update(blinkMsg);
    DEBUG_ISR(" upd anchor : %d x:%d y:%d dist: %d", address, rap->_x, rap->_y,
              rap->_distance);
  }
}
...
void DWM1000_Tag::listAnchors(std::string& output) {
  ...
  for (uint32_t i = 0; i < MAX_ANCHORS; i++)
    if (anchors[i]._address != 0) {
      anchor = stringFormat("{ address=%d, x=%d , y = %d, distance = %d  } ",
...
FrameType DWM1000_Tag::readMsg(const dwt_callback_data_t* signal) {
  ...
  if (frameLength <= sizeof(_dwmMsg)) {
    ...
    if (ft == FT_BLINK) {
      ...
      DEBUG_ISR(" blink %d : %d : %s", _blinkMsg.getSrc(), _blinkMsg.sequence,
                stateString());
      _blinks++;
    } else if (ft == FT_POLL) {
      memcpy(_pollMsg.buffer, _dwmMsg.buffer, sizeof(_pollMsg));
      DEBUG_ISR(" poll %d : %d : %s", _pollMsg.getSrc(), _pollMsg.sequence,
                stateString());
      _polls++;
    } else if (ft == FT_RESP) {
      memcpy(_respMsg.buffer, _dwmMsg.buffer, sizeof(_respMsg));
      DEBUG_ISR(" resp %d : %d : %s ", _respMsg.getSrc(), _respMsg.sequence,
                stateString());
      _resps++;
    } else if (ft == FT_FINAL) {
      memcpy(_finalMsg.buffer, _dwmMsg.buffer, sizeof(_finalMsg));
      DEBUG_ISR(" final %d : %d : %s", _finalMsg.getSrc(), _finalMsg.sequence,
                stateString());
      _finals++;
    } else {
      WARN_ISR(" unknown frame type %X:%X : %s", _dwmMsg.fc[0], _dwmMsg.fc[1],
               stateString());
    }
    return ft;
  } else {
    WARN_ISR("WARN invalid length %d : hdr %X:%X : %s", frameLength,
             _dwmMsg.fc[0], _dwmMsg.fc[1], stateString());
    return FT_UNKNOWN;
  }
}
