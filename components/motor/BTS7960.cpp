...
static Register
    pwm_fh0_cfg0("PWM_FH0_CFG0_REG",
                 "- - - - - - - - - - - - - - - - - - - - - - - - - - - "
                 "PWM_FH0_FORCE_OST "
                 "PWM_FH0_FORCE_CBC "
                 "+ PWM_FH0_CBCPULSE "
                 "PWM_FH0_CLR_OST");
static Register pwm_fh0_status_reg(
    "PWM_FH0_STATUS_REG",
    "- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - "
    "PWM_FH0_OST_ON "
    "PWM_FH0_CBC_ON");
...
static Register
    pwm_fh1_cfg1("PWM_FH1_CFG1_REG",
                 "- - - - - - - - - - - - - - - - - - - - - - - - - - - "
                 "PWM_FH1_FORCE_OST "
                 "PWM_FH1_FORCE_CBC "
                 "+ PWM_FH1_CBCPULSE "
                 "PWM_FH1_CLR_OST");
static Register pwm_fh1_status_reg(
    "PWM_FH1_STATUS_REG",
    "- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - "
    "PWM_FH1_OST_ON "
    "PWM_FH1_CBC_ON");
static Register
    pwm_fh2_cfg2("PWM_FH2_CFG2_REG",
                 "- - - - - - - - - - - - - - - - - - - - - - - - - - - "
                 "PWM_FH2_FORCE_OST "
                 "PWM_FH2_FORCE_CBC "
                 "+ PWM_FH2_CBCPULSE "
                 "PWM_FH2_CLR_OST");
static Register pwm_fh2_status_reg(
    "PWM_FH2_STATUS_REG",
    "- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - "
    "PWM_FH2_OST_ON "
    "PWM_FH2_CBC_ON");
...
static Register reg_prescaler("PWM_CLK_CFG_REG",
                              ...
                              "+ + + + + + + PWM_CLK_PRESCALE");
static Register
    reg_fault_detect("PWM_FAULT_DETECT_REG",
                     "- - - - - - - - - - - - - - - - - - - - - - - "
                     "PWM_EVENT_F2 PWM_EVENT_F1 PWM_EVENT_F0 PWM_F2_POLE "
                     "PWM_F1_POLE PWM_F0_POLE PWM_F2_EN PWM_F1_EN PWM_F0_EN");
static Register
    timer0_status_reg("PWM_TIMER0_STATUS_REG",
...
void BTS7960::setOutput(float dutyCycle) {
  //	INFO("MCPWM[%d] PWM=%.1f
  //last=%.1f",_mcpwm_num,dutyCycle,_lastDutyCycle);
  setDirection(dutyCycle);
  if (abs(_lastDutyCycle - dutyCycle) < 0.5)
    return;
  //   dutyCycle =  lastDutyCycle*(1-weight)+dutyCycle*weight;
  /*    if ((dutyCycle < 0 && _lastDutyCycle > 0) || (dutyCycle > 0 &&
     _lastDutyCycle < 0)) { left(0); right(0); } else*/
  if (dutyCycle < 0) {
    left(-dutyCycle);
  } else if (dutyCycle > 0) {
    right(dutyCycle);
...
Erc BTS7960::initialize() {
  ...
  INFO(" BTS7960 PWM[%d] PWM : GPIO_%d,%d enable : GPIO_%d,%d ", _mcpwm_num,
       _pinPwmLeft, _pinPwmRight, _pinLeftEnable.getPin(),
       _pinRightEnable.getPin());
...
void BTS7960::setMaxPwm(uint32_t max) { MAX_PWM = max; }
