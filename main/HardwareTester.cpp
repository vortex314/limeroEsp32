...
Reg HardwareTester::regs[] = {
    {"Configuration of the prescaler", "PWM_CLK_CFG_REG",
     "- - - - - - - - - - - - - - - - - - - - - - - - + + + + + + + "
     "PWM_CLK_PRESCALE",
     (uint32_t*)MCPWM_CLK_CFG_REG(PWM)},
    {"Timer period and update method", "PWM_TIMER0_CFG0_REG",
     "- - - - - - + PWM_TIMER0_PERIOD_UPMETHOD + + + + + + + + + + + + + + + "
     "PWM_TIMER0_PERIOD + + + + + + + PWM_TIMER0_PRESCALE",
     (uint32_t*)MCPWM_TIMER0_CFG0_REG(PWM)},
    {"Working mode and start/stop control", "PWM_TIMER0_CFG1_REG",
     "- - - - - - - - - - - - - - - - - - - - - - - - - - - + PWM_TIMER0_MOD + "
     "+ PWM_TIMER0_START",
     (uint32_t*)MCPWM_TIMER0_CFG1_REG(PWM)},
    {"Synchronization settings", "PWM_TIMER0_SYNC_REG",
     "- - - - - - - - - - - + + + + + + + + + + + + + + + + PWM_TIMER0_PHASE + "
     "PWM_TIMER1_SYNCO_SEL PWM_TIMER1_SYNC_SW PWM_TIMER1_SYNCI_EN",
     (uint32_t*)MCPWM_TIMER0_SYNC_REG(PWM)},
    {"Timer status", "PWM_TIMER0_STATUS_REG",
     "- - - - - - - - - - - - - - - PWM_TIMER0_DIRECTION + + + + + + + + + + + "
     "+ + + + PWM_TIMER0_VALUE",
    ...
    {"Transfer status and update method for time stamp registers A and B",
     ...
     "- - - - - - - - - - - - - - - - - - - - - - PWM_GEN0_B_SHDW_FULL "
     "PWM_GEN0_A_SHDW_FULL + + + PWM_GEN0_B_UPMETHOD + + + PWM_GEN0_A_UPMETHOD",
     (uint32_t*)MCPWM_GEN0_STMP_CFG_REG(PWM)},
    {"Configure capture timer", "PWM_CAP_TIMER_CFG_REG",
     "- - - - - - - - - - - - - - - - - - - - - - - - - - PWM_CAP_SYNC_SW + + "
     "PWM_CAP_SYNCI_SEL PWM_CAP_SYNCI_EN PWM_CAP_TIMER_EN",
     (uint32_t*)MCPWM_CAP_TIMER_CFG_REG(PWM)},
    {"Phase for capture timer sync", "PWM_CAP_TIMER_PHASE_REG",
     "- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - ",
     (uint32_t*)MCPWM_CAP_TIMER_PHASE_REG(PWM)},
    {"Capture channel 0 configuration and enable", "PWM_CAP_CH0_CFG_REG",
     "- - - - - - - - - - - - - - - - - - - PWM_CAP0_SW PWM_CAP0_IN_INVERT + + "
     "+ + + + + PWM_CAP0_PRESCALE + PWM_CAP0_MODE PWM_CAP0_EN",
...
void IRAM_ATTR HardwareTester::isrCaptureHandler(void* pv) {
  ...
  if (mcpwm_intr_status & CAP0_INT_EN) {
    uint32_t capt =
        mcpwm_capture_signal_get_value(hw->mcpwm_num, MCPWM_SELECT_CAP0);
    hw->capts = capt - prevCapt;
    prevCapt = capt;
  }
  MCPWM[hw->mcpwm_num]->int_clr.val =
      mcpwm_intr_status;  // Clear interrupt (clr)
}
...
int HardwareTester::captureTest() {
  ...
  // Set ISR Handler
  rc = mcpwm_isr_register(mcpwm_num, isrCaptureHandler, this,
